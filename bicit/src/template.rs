...
impl Template {
    /// Create a template from SVG content string
    pub fn new(content: impl Into<String>) -> Template {
        Template {
...
"#;
        ...
        context.load().unwrap();
        let template = Template::new(xml);
        let result = template.apply_context(&context).unwrap();
        let expected = r#"<text
       xml:space="preserve"
       style="font-style:normal;font-weight:normal;font-size:10.5833px;line-height:1.25;font-family:sans-serif;letter-spacing:0px;word-spacing:0px;fill:#000000;fill-opacity:1;stroke:none;str
oke-width:0.264583"
